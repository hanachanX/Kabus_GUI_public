...
class SpoofDetector:
    ...
    def _detect_relocation(self, snap: Snapshot) -> Optional[State]:
        ...
        for i in range(1,len(buf)):
            if buf[i].ask - buf[i-1].ask >= min_step:
                steps_s += 1
        for i in range(1,len(buf)):
            if buf[i-1].bid - buf[i].bid >= min_step:
                steps_b += 1
        def stacked(levels):
            if not levels:
                return False
        ...
        side=None
        if steps_s>=need and stacked(snap.levels_a):
            side='S'
        elif steps_b>=need and stacked(snap.levels_b):
            side='B'
        if not side:
            return None
    ...
    def _detect_flip(self, snap: Snapshot) -> Optional[State]:
        ...
        if not (big_drop_s or big_drop_b):
            return None
        tr = [t for t in self.trades if (snap.t - t['t']) <= follow]
        if not tr:
            return None
        opp=None
        if big_drop_s and any(t['side']=='B' for t in tr):
            opp='S'
        if big_drop_b and any(t['side']=='S' for t in tr):
            opp='B'
        if not opp:
            return None
